...
import (
	...
	"github.com/manifest-Network/manifest/v4/app/ante"
...
// Example:
// 21/10 Employee joins manifest and vesting starts
...
func delegate(clawbackAccount *types.ClawbackVestingAccount, amount int64) error {
	...
	s.Require().NoError(err)
	//
	val, err := sdk.ValAddressFromBech32("manifestvaloper1z3t55m0l9h0eupuz3dp5t5cypyv674jjn4d6nn")
	s.Require().NoError(err)
